...
public class RenderUtil implements ClientModInitializer {
    ...
    private static final Vector4f COLOR_MODULATOR = new Vector4f(1f, 1f, 1f,
            1f);
    ...
    // :::custom-pipelines:extraction-phase
    private void renderProjectilePrediction(WorldRenderContext context) {
        if (buffer == null) {
            buffer = new BufferBuilder(allocator,
                    FILLED_THROUGH_WALLS.getVertexFormatMode(),
                    FILLED_THROUGH_WALLS.getVertexFormat());
        }
        boolean rendered = ProjectilePredictionUtil.render(context);
        if (!rendered) {
            buffer = null;
        }
    }

    public void renderFilledBox(Matrix4fc positionMatrix,
                                float minX, float minY, float minZ, float maxX,
                                float maxY, float maxZ, float red, float green,
                                float blue, float alpha) {
        if (buffer == null) return;
        renderFilledBox(positionMatrix, buffer, minX, minY, minZ, maxX, maxY,
                maxZ, red, green, blue, alpha);
    }
    ...
    private void renderFilledBox(Matrix4fc positionMatrix, BufferBuilder buffer,
                                 ...
                                 float blue, float alpha) {
        // Front Face
        buffer.addVertex(positionMatrix, minX, minY, maxZ)
                .setColor(red, green, blue, alpha);
        buffer.addVertex(positionMatrix, maxX, minY, maxZ)
                .setColor(red, green, blue, alpha);
        buffer.addVertex(positionMatrix, maxX, maxY, maxZ)
                .setColor(red, green, blue, alpha);
        buffer.addVertex(positionMatrix, minX, maxY, maxZ)
                .setColor(red, green, blue, alpha);

        // Back face
        buffer.addVertex(positionMatrix, maxX, minY, minZ)
                .setColor(red, green, blue, alpha);
        buffer.addVertex(positionMatrix, minX, minY, minZ)
                .setColor(red, green, blue, alpha);
        buffer.addVertex(positionMatrix, minX, maxY, minZ)
                .setColor(red, green, blue, alpha);
        buffer.addVertex(positionMatrix, maxX, maxY, minZ)
                .setColor(red, green, blue, alpha);
    ...
    private void drawFilledThroughWalls(Minecraft client,
        ...
        // Rotate the vertex buffer so we are less likely to use buffers that
        // the GPU is using
        vertexBuffer.rotate();
        buffer = null;
    }

    private GpuBuffer upload(MeshData.DrawState drawParameters,
                             VertexFormat format, MeshData builtBuffer) {
        ...
        if (vertexBuffer == null || vertexBuffer.size() < vertexBufferSize) {
            ...
            vertexBuffer = new MappableRingBuffer(
                    () -> MineipClient.MOD_ID + " example render pipeline",
    ...
    private static void draw(Minecraft client, RenderPipeline pipeline,
        ...
        if (pipeline.getVertexFormatMode() == VertexFormat.Mode.QUADS) {
            ...
            builtBuffer.sortQuads(allocator,
        ...
        try (RenderPass renderPass = RenderSystem.getDevice()
                .createCommandEncoder()
                .createRenderPass(() -> MineipClient.MOD_ID +
                                " example render pipeline rendering",
            ...
            // The base vertex is the starting index when we copied the data
            // into the vertex buffer divided by vertex size
            //noinspection ConstantValue
            renderPass.drawIndexed(0 / format.getVertexSize(), 0,
                    drawParameters.indexCount(), 1);
        }
